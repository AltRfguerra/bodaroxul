...
(function ($) {
  ...
  $.fn._parallax = function (intensity) {
    ...
    $this.each(function () {
      ...
      // Disable parallax on ..
      if (_browser.browser.name == "ie" ||
      // IE
      _browser.browser.name == "edge" ||
      // Edge
      window.devicePixelRatio > 1 ||
      // Retina/HiDPI (= poor performance)
      _browser.browser.mobile)
        // Mobile devices
        off();
        // Enable everywhere else.
      else {
        _breakpoints.breakpoints.on(">large", on);
        _breakpoints.breakpoints.on("<=large", off);
      }
    });
  ...
  _breakpoints.breakpoints.on(">medium", function () {
    // NavPanel -> Nav.
  ...
  if (_browser.browser.os == "wp" && _browser.browser.osVersion < 10) $navPanel.css("transition", "none");
  ...
  if ($intro.length > 0) {
    ...
    _breakpoints.breakpoints.on(">small", function () {
      $main.unscrollex();
      $main.scrollex({
        mode: "bottom",
        top: "80vh",
